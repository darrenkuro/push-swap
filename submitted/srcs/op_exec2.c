...
/*   Updated: 2023/05/25 01:15:15 by dlu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	op_exec_ra(t_data *data, int index)
{
	...
	if (index >= 0 && index < MAX_SORT)
		ft_lstadd_back(&data->op[index], ft_lstnew("ra"));
	return (TRUE);
}
...
int	op_exec_rb(t_data *data, int index)
{
	...
	if (index >= 0 && index < MAX_SORT)
		ft_lstadd_back(&data->op[index], ft_lstnew("rb"));
	return (TRUE);
}
...
int	op_exec_rr(t_data *data, int index)
{
	...
	op_exec_rb(data, -1);
	if (index >= 0 && index < MAX_SORT)
		ft_lstadd_back(&data->op[index], ft_lstnew("rr"));
	return (TRUE);
}
